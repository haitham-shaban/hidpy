function uv = compute_flow(mask, this, init, gt)
%
...
% plot for Haitham
% for k = 1:length(org_pyramid_images)
%     for kk = 1:size(org_pyramid_images{k},3)
%     f = figure; hold on
%     imshow(org_pyramid_images{k}(:,:,kk) / max(max(org_pyramid_images{k}(:,:,kk)))), 
%     colormap gray
%     axis off, axis tight
% %     truesize
%     set(gcf, 'color', 'w')
%     f.InnerPosition(3:4) = ones(1,2).*max(f.InnerPosition(3:4));
%     f.OuterPosition(3:4) = ones(1,2).*max(f.OuterPosition(3:4));
%     export_fig(['level' num2str(k) 'im' num2str(kk) '.emf'])
%     end
% end
...
% this.gnc_iters     =10; %%% FOR TESTING ISSUE
for ignc = 1:this.gnc_iters
    ...
    for l = pyramid_levels:-1:1
        ...
        if l ==1
        uv = compute_flow_base(small, uv, ignc, count);
        else
            uv = compute_flow_base(small, uv, ignc, count);
        end
        %%% FOR TESTING ISSUE
        ...
        %               this.alpha
        %           figure(ignc);
        %           imagesc(sqrt(uv(:,:,1).^2+uv(:,:,2).^2).*imresize(mask, [size(uv,1), size(uv,2)]))
        %           end
%             %% for visualization  - deleteme
%     save(['C:\Users\Roman\MEGA\Conferences\2020_01_Seeing_and_decoding_nucelar_function_and_structure_Barcelona_Spain\UV_iteration' num2str(ignc) '_level_' num2str(count) '.mat'], 'uv')

    end % for l over pyramind levels
