function uv = compute_flow_base(this, uv, ignc, count)
%%
...
% THIS SOFTWARE.        
  ...
  % Spatial
  %%% EDIT 22/05/2017 for use on cluster
...
%   end;
  ...
  for i = 1:this.max_iters
    ...
    %%%%
% % %     this.deriv_filter = fspecial('gaussian', 20, 5);%1/512*[-1 -4 -5 0 5 4 1; -4 -16 -20 0 20 16 4; -6 -24 -30 0 30 24 6; -4 -16 -20 0 20 16 4; -1 -4 -5 0 5 4 1];
    %%%%
    [It, Ix, Iy] = partial_deriv(this.images, uv, this.interpolation_method, this.deriv_filter);
%     %% deleteme
%     i
%     if i == this.max_iters
%         save(['C:\Users\Roman\MEGA\Conferences\2020_01_Seeing_and_decoding_nucelar_function_and_structure_Barcelona_Spain\It_iteration' num2str(ignc) '_level_' num2str(count) '.mat'], 'It')
%     end
    %%% FOR TESTING ISSUE
% % %     sigma = 1;
% % %                                 filterSize = 2*ceil(2*sigma) + 1; % as used by imgaussfilt.m
% % %                             filt = fspecial('gaussian',filterSize,sigma);
% % %                             It = imfilter(It,filt,'replicate');
% % % if i==this.max_iters
% % %     figure,% subplot(131), imagesc(Ix), subplot(132), imagesc(Iy), subplot(133), 
% % %     imagesc(It),
% % % end
...
%         end
    ...
    % Update flow fileds
    uv = uv + duv;
%   [x,y] = meshgrid(1:size(uv,2), 1:size(uv,1));
%   figure, hold on, quiverc_direction(x,y,uv(:,:,1),uv(:,:,2)), axis ij
    
  end  
  
%   [x,y] = meshgrid(1:size(uv,2), 1:size(uv,1));
%   figure, quiver(x,y,uv(:,:,1),uv(:,:,2))
