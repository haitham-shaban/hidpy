function [grid,analog]=simStacksCore(emitter_position, frames,Optics,Cam,Fluo,Grid,intensity_peak_mode,tutorial)
...
if Fluo.Toff == 0
    emitter_brightness = Fluo.Ion.*ones(size(emitter_position,1),frames);
    % emitter_brightness = Fluo.Ion.*repmat(rand(size(emitter_position,1),1),1,frames);
else
...
for m=1:Nemitters
%         waitbar(m/Nemitters);
% dispstat(sprintf(['Diffraction: ' num2str(str2num(sprintf('%.1f',m/Nemitters*100))) '%']))    % Discrete Grid
for t = 1:frames
    [x,y]=ind2sub(size(grid),find((gridY - emitter_position(m,1,t)).^2 + (gridX - emitter_position(m,2,t)).^2 <=  (r)^2 == 1));

    for k=1:length(x)
        grid(x(k),y(k),t)= squeeze(grid(x(k),y(k),t)) + emitter_brightness(m,t)'.*...
                         (squeeze(erf((x(k)-emitter_position(m,1,t)+0.5)/(sqrt(2)*s_xy))) - squeeze(erf((x(k)-emitter_position(m,1,t)-0.5)/(sqrt(2)*s_xy)))).*...
                         (squeeze(erf((y(k)-emitter_position(m,2,t)+0.5)/(sqrt(2)*s_xy))) - squeeze(erf((y(k)-emitter_position(m,2,t)-0.5)/(sqrt(2)*s_xy))));
    end
    clear x y;
end
...
clear Ion; clear Ton; clear Tbl;
photons=cumsum(photons(:));
% one point over two has no photons, this is 
% the period in the off state. All others have photons for successive
% duratoins of Ton.
photons=diff(interp1(times,photons,0:frames,'linear',0));
% I = 1./(1+(99/Tbl).*(0:length(photons)-1)); % bleaching follows this distribution: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3207365/
